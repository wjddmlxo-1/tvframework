...
- [CM_IDS (ID 생성 서비스)](#cm_ids-id-생성-서비스)
...
**용도**: 애플리케이션 공통 ID 채번(`TableIdGnrServiceImpl` 등).


| 컬럼         | 타입          | 제약     | 기본값   | 설명      |
| ---------- | ----------- | ------ | ----- | ------- |
| TABLE_NAME | VARCHAR(20) | PK, NN |       | 채번 키(행 식별자) |
| NEXT_ID    | NUMERIC(30) | NN     | 0  | 다음 ID 값 |
| CREATION_ID | VARCHAR(20) |        |      | 생성자 |
| CREATION_DT | TIMESTAMP   |        | CURRENT_TIMESTAMP | 생성일 |
| UPDATE_ID   | VARCHAR(20) |        |      | 수정자 |
| UPDATE_DT   | TIMESTAMP   |        |      | 수정일 |
