package tvframework.let.cop.smt.sim.web;
...
import tvframework.com.cmm.service.FileMngUtil;
...
import tvframework.com.cmm.web.FileDownloadController;
import tvframework.let.cop.smt.sim.service.IndvdlSchdulManageService;
import tvframework.let.cop.smt.sim.service.IndvdlSchdulManageVO;
import tvframework.let.cop.smt.sim.service.ScheduleSearchVO;
...
@Tag(name="IndvdlSchdulManageApiController",description = "일정관리")
public class IndvdlSchdulManageApiController {

	private final DefaultBeanValidator beanValidator;
	private final IndvdlSchdulManageService egovIndvdlSchdulManageService;
    ...
    @Operation(
			...
			tags = {"IndvdlSchdulManageApiController"}
	)
	@ApiResponses(value = {
	...
	public ResultVO EgovIndvdlSchdulManageDetail(
		...
		if (scheduleDetail.getAtchFileId() != null && !scheduleDetail.getAtchFileId().isEmpty()) {
			...
			for (FileVO file : resultFiles) {
				...
				file.setAtchFileId(Base64.getEncoder().encodeToString(cryptoService.encrypt(toEncrypt.getBytes(),FileDownloadController.ALGORITM_KEY)));
			}
