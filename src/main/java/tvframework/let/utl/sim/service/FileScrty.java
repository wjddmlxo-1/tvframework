package tvframework.let.utl.sim.service;
...
public class FileScrty {
	...
	public static boolean encryptFile(String source, String target) throws Exception {
		...
		String sourceFile = WebUtil.filePathBlackList(source.replace("\\", FILE_SEPARATOR).replace("/", FILE_SEPARATOR));
		String targetFile = WebUtil.filePathBlackList(target.replace("\\", FILE_SEPARATOR).replace("/", FILE_SEPARATOR));
